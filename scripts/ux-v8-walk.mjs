#!/usr/bin/env node
/**
 * UX Walk v8 — 2026-05-05
 * Eighth UX pass. Post Wave 22 fixes:
 *   - e48ff45d: Director occupant_id → display name resolution (WS-22-C)
 *   - 946590bb: Hairline 2nd pass sweep (WS-22-D) + personal treasury copy (WS-22-E)
 *   - 019dd063: entity.type === 'human' fix for personal detection
 *
 * Live bundle: index-5sn369oX.js (Wave 22)
 *
 * Per-fix verification:
 *   1. Director occupant: must show display name, not raw UUID (WS-22-C)
 *   2. Hairlines: target drop from 15/27 → ~10/27 routes (WS-22-D)
 *   3. Personal /me/treasury copy: "account" not "Company" (WS-22-E + 019dd063)
 *
 * v7 carry-forward:
 *   - AEQI_UPPERCASE_STRUCTURAL FP on session-rail ASIDE — excluded below
 ...
 *   Raw JSON   → /home/claudedev/aeqi/.observations/ux-v8/raw.json
 *
 * Usage:
 *   AEQI_WEB_SECRET=... node scripts/ux-v8-walk.mjs
 */
...
const SCREENSHOT_DIR = join(REPO_ROOT, ".observations", "ux-v8");
...
console.log(`Bundle under test: index-5sn369oX.js (Wave 22)`);
...
// v8 update: AEQI_UPPERCASE_STRUCTURAL now excludes session-rail elements to kill FPs
...
async function visitRoute(context, route) {
  ...
  try {
    ...
    // ── v8 specific checks ──
    try {
      const v8Issues = await page.evaluate(() => {
        const issues = [];
        const allText = document.body ? (document.body.innerText || "") : "";
        const path = window.location.pathname;

        // v8-A: Governance copy — confirm still clean from Wave 20
        if (allText.includes("See proposals") && allText.includes("Phase 2")) {
          issues.push({ code: "BUILD_NOTE_EXPOSED", severity: "P2",
            detail: '"See proposals...Phase 2" internal note still present — regression' });
        }
        if (path.includes("governance") && allText.includes("No proposals yet")) {
          issues.push({ code: "GOV_COPY_FIXED", severity: "info",
            detail: '"No proposals yet" copy confirmed — Wave 20 governance fix still present' });
        }

        // v8-B: Director occupant — WS-22-C fix verification
        // Expect: display name (not raw UUID, not "Unoccupied")
        if (path.includes("/roles") && !path.match(/\/roles\/[0-9a-f]/)) {
          const uuidRe = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/;
          const allEls = Array.from(document.querySelectorAll("*"));
          const dirEl = allEls.find(el => (el.childNodes.length <= 3) && (el.innerText || "").trim() === "Director");
          if (dirEl) {
            const card = dirEl.closest("[class]") || dirEl.parentElement;
            const cardText = card ? (card.innerText || "") : "";
            if (uuidRe.test(cardText)) {
              issues.push({ code: "DIRECTOR_UUID_OCCUPANT", severity: "P2",
                detail: 'Director card still shows raw UUID (WS-22-C fix not landed). Text: "' + cardText.trim().slice(0, 120) + '"' });
            } else if (/Unoccupied/i.test(cardText)) {
              issues.push({ code: "DIRECTOR_OCCUPANT_UNOCCUPIED", severity: "info",
                detail: 'Director shows "Unoccupied" — UUID null-coalesc path, not the WS-22-C resolve path' });
            } else {
              // Has text but not UUID and not Unoccupied — assume display name resolved
              const occupantText = cardText.trim().slice(0, 100);
              issues.push({ code: "DIRECTOR_OCCUPANT_RESOLVED", severity: "info",
                detail: 'Director occupant resolved (not UUID, not Unoccupied). Card text: "' + occupantText + '"' });
            }
          } else {
            issues.push({ code: "DIRECTOR_EL_NOT_FOUND", severity: "info",
              detail: "Director role element not found on roles page — may be empty or different layout" });
          }
        }

        // v8-C: Personal treasury copy — WS-22-E fix verification
        if (path.startsWith("/me") && path.includes("treasury")) {
          ...
          if (hasCompanyCopy) {
            issues.push({ code: "PERSONAL_TREASURY_COMPANY_COPY", severity: "P3",
              detail: 'Personal treasury still says "This Company..." — WS-22-E fix not landed' });
          }
          if (hasAccountCopy) {
            issues.push({ code: "PERSONAL_TREASURY_FIXED", severity: "info",
              detail: 'Personal treasury uses "account" copy — WS-22-E fix confirmed' });
          }
          if (!hasCompanyCopy && !hasAccountCopy) {
            issues.push({ code: "PERSONAL_TREASURY_COPY_UNKNOWN", severity: "info",
              detail: 'Treasury copy ambiguous — sample: "' + allText.slice(0, 200).replace(/\n/g, " ") + '"' });
          }
        }

        // v8-D: AEQI uppercase in company-overview identity tile (DB-stored pre-fix)
        ...
        // v8-E: Session-rail AEQI — now just info (structural detector excludes rail in v8)
        if (path.match(/\/agents\//) && (path.endsWith("/agents/" + path.split("/agents/")[1].split("/")[0]) || path.includes("/sessions"))) {
          ...
          if (sessionRailEl) {
            ...
            issues.push({ code: "SESSION_RAIL_AEQI_INFO", severity: "info",
              detail: "Session rail AEQI count: " + railAeqi + "x (excluded from structural check in v8)" });
          }
        }

        // v8-F: Wallet upgrade section — /me/settings
        ...
        // v8-G: Hairline count per route — separate breakdown info
        let hlCount = 0;
        for (const el of Array.from(document.querySelectorAll("*")).slice(0, 400)) {
          const s = window.getComputedStyle(el);
          if (parseFloat(s.borderTopWidth) === 1
        ...
        issues.push({ code: "HAIRLINE_COUNT_V8", severity: "info",
          detail: "v8 hairline count: " + hlCount + " (threshold >5 = P2)" });

        return issues;
      });
      antiPatterns = antiPatterns.concat(v8Issues);
    } catch (e) {
      console.warn(`  v8 checks eval error on ${route.label}: ${e.message}`);
    }
