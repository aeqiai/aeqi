#!/usr/bin/env node
/**
 * UX Walk v10 — 2026-05-05
 * Tenth UX pass. Post Wave 23 changes since v9:
 *   - 5ba89fe (aeqi-landing): Fix "AEQI Entity & AA" → "aeqi Entity & AA" in docs nav
 *   - 3dfcc03 (aeqi-platform): Register POST /api/wallet/upgrade-to-passkey stub (501)
 *     NOTE: stub committed but NOT yet deployed to live binary (binary from 03:55,
 *     commit at 07:12 — detected via stat + service start time). Live still returns 401.
 *
 * v9 score: 9.3/10
 * Live bundle: index-CDVRX8gJ.js (unchanged — app bundle not touched since Wave 23)
 *
 * Per-fix verification:
 *   1. 5ba89fe: landing-docs nav — "aeqi Entity & AA" (lowercase) confirmed via curl
 *      Ground truth: page body text must NOT contain "AEQI Entity & AA" (uppercase AEQI)
 *   2. 3dfcc03: wallet upgrade stub — tests live API response
 *      Ground truth: with auth, POST /api/wallet/upgrade-to-passkey returns 501
 *      (NOT 401 "missing authorization header")
 *   3. Hairlines: any further drop from v9's 13/27 routes (unchanged from v8)
 *
 * v9 carry-forward:
 *   - AEQI uppercase in DB-stored identity idea for Luca Eich (company-overview)
 *   - Hairline count 13/27 routes unchanged (Wave 23 had no hairline work)
 *
 * Detection improvements over v9:
 *   - v10-A: docs nav check — verify "aeqi Entity" (lowercase) in ASIDE/NAV
 *   - v10-B: wallet upgrade API probe — checks live 501 vs 401 response code
 *   - v10-C: HAIRLINE_COUNT_V10 tag for v10 comparison
 *
 * Output:
 *   Screenshots → /home/claudedev/aeqi/.observations/ux-v10/
 *   Raw JSON   → /home/claudedev/aeqi/.observations/ux-v10/raw.json
 *
 * Usage:
 *   AEQI_WEB_SECRET=... node scripts/ux-v10-walk.mjs
 */
...
const SCREENSHOT_DIR = join(REPO_ROOT, ".observations", "ux-v10");
...
console.log(`Bundle under test: index-CDVRX8gJ.js (Wave 23 — unchanged from v9)`);

// ── Wallet upgrade API probe (out-of-band, before browser walk) ───────────────
...
console.log(
  walletUpgradeIs501
    ? "  ✓ Returns 501 — 3dfcc03 DEPLOYED. Frontend graceful-degrade will fire."
    : walletUpgradeIs401
      ? "  ✗ Still 401 — 3dfcc03 NOT YET DEPLOYED. Binary predates commit (built 03:55, commit 07:12)."
...
// ── Anti-pattern detector (inherited from v9 — no core changes) ────────────────
...
// ── Core visit function ───────────────────────────────────────────────────────
async function visitRoute(context, route, walletProbeResult) {
  ...
  try {
    ...
    // ── v10 specific checks ──
    try {
      const v10Issues = await page.evaluate(
        (args) => {
          const { walletProbeStatus, walletProbeBody } = args;
          const issues = [];
          const allText = document.body ? (document.body.innerText || "") : "";
          const path = window.location.pathname;

          // v10-A: docs nav — "aeqi Entity & AA" lowercase check (5ba89fe)
          // The structural AEQI detector catches AEQI uppercase in nav/ASIDE.
          // v10 adds a positive confirmation: look for "aeqi Entity" (lowercase) in the Architecture nav section.
          if (window.location.hostname.includes("aeqi.ai") && !window.location.hostname.includes("app.")) {
            // Check if this is the docs page
            const navEls = Array.from(document.querySelectorAll("nav, [class*=nav], aside, ASIDE"));
            let foundLowercaseAeqiEntity = false;
            let foundUppercaseAeqiEntity = false;
            for (const el of navEls) {
              const t = el.innerText || "";
              if (t.includes("aeqi Entity")) foundLowercaseAeqiEntity = true;
              if (t.includes("AEQI Entity")) foundUppercaseAeqiEntity = true;
            }

            if (foundUppercaseAeqiEntity) {
              issues.push({ code: "DOCS_NAV_AEQI_UPPERCASE", severity: "P1",
                detail: "5ba89fe NOT LANDED: docs nav still shows 'AEQI Entity & AA' (uppercase). Fix not active." });
            } else if (foundLowercaseAeqiEntity) {
              issues.push({ code: "DOCS_NAV_AEQI_FIXED", severity: "info",
                detail: "5ba89fe CONFIRMED: docs nav shows 'aeqi Entity & AA' (lowercase aeqi) — fix live." });
            } else {
              issues.push({ code: "DOCS_NAV_AEQI_NOT_FOUND", severity: "info",
                detail: "Docs nav: 'aeqi Entity' not found in nav elements (may be non-docs page or element selector miss)." });
            }
          }

          // v10-B: wallet upgrade API probe result (injected from out-of-band probe)
          if (path.startsWith("/me") && path.includes("settings")) {
            if (walletProbeStatus === 501) {
              issues.push({ code: "WALLET_UPGRADE_501_DEPLOYED", severity: "info",
                detail: "3dfcc03 CONFIRMED DEPLOYED: POST /api/wallet/upgrade-to-passkey returns 501. Frontend graceful-degrade fires correctly." });
            } else if (walletProbeStatus === 401) {
              issues.push({ code: "WALLET_UPGRADE_STILL_401", severity: "P2",
                detail: "3dfcc03 NOT YET DEPLOYED: POST /api/wallet/upgrade-to-passkey still returns 401. Binary predates commit. Frontend shows error banner instead of graceful state." });
            } else {
              issues.push({ code: "WALLET_UPGRADE_UNEXPECTED", severity: "info",
                detail: "Wallet upgrade probe returned unexpected status " + walletProbeStatus + ". Body: " + walletProbeBody });
            }

            // v10-B2: wallet section present check (v9 carry-forward)
          ...
          // v10-C: Hairline count (v10 tag for comparison)
          let hlCount = 0;
          for (const el of Array.from(document.querySelectorAll("*")).slice(0, 400)) {
            const s = window.getComputedStyle(el);
            if (parseFloat(s.borderTopWidth) === 1
          ...
          issues.push({ code: "HAIRLINE_COUNT_V10", severity: "info",
            detail: "v10 hairline count: " + hlCount + " (threshold >5 = P2)" });

          // v10-D: Director card check (v9 carry-forward, same logic)
            ...
            if (allText.includes(USER_UUID)) {
              ...
              if (directorIdx >= 0 && proximity < 200) {
                issues.push({ code: "DIRECTOR_UUID_IN_LIST_VIEW", severity: "P2",
                  detail: 'Director card still shows raw UUID (regression check). UUID at proximity ' + proximity + ' chars from "Director".' });
              }
            } else {
              const directorIdx = allText.indexOf("Director");
              if (directorIdx >= 0) {
                ...
                if (!hasAnyUuid) {
                  issues.push({ code: "DIRECTOR_NAME_RESOLVED", severity: "info",
                    detail: 'WS-23-B still passing: Director occupant resolved. Context: "' + context.replace(/\n/g, " ").trim().slice(0, 100) + '"' });
                }
              }
            }
          }

          // v10-E: Personal treasury copy (v9 carry-forward regression check)
          if (path.startsWith("/me") && path.includes("treasury")) {
            ...
            const hasAccountCopy = allText.includes("This account") || allText.includes("account isn");
            if (hasCompanyCopy) {
              issues.push({ code: "PERSONAL_TREASURY_COMPANY_COPY", severity: "P3",
                detail: "WS-23-C REGRESSED: Personal treasury shows 'This Company...' copy." });
            } else if (hasAccountCopy) {
              issues.push({ code: "PERSONAL_TREASURY_FIXED", severity: "info",
                detail: "WS-23-C still passing: Personal treasury uses 'account' copy." });
            }
          }

          // v10-F: Company treasury regression check
          if (path.includes("/c/") && path.includes("treasury")) {
            const hasCompanyCopy = allText.includes("This Company");
            if (hasCompanyCopy) {
              issues.push({ code: "COMPANY_TREASURY_COPY_CORRECT", severity: "info",
                detail: "Company treasury 'Company' copy intact — no regression." });
            }
          }

          // v10-G: AEQI uppercase in company-overview identity tile (DB-stored, P1 open issue)
            ...
            for (const el of missionEls) {
              if (/\\bAEQI\\b/.test(el.innerText || "")) {
                ...
                issues.push({ code: "MISSION_AEQI_UPPERCASE", severity: "P2",
                  detail: "Identity tile still has uppercase AEQI (DB-stored pre-fix record) — P1 carry-forward" });
                break;
              }
            }
            if (!missionAeqi) {
          ...
          // v10-H: Raw UUID count in page text (regression check)
          ...
          if (allUuids.length > 0) {
            issues.push({ code: "UUID_IN_PAGE_TEXT", severity: "info",
              detail: allUuids.length + " UUID(s) found in page text (may be user content): " + allUuids.slice(0, 3).join(", ") });
          }

          return issues;
        },
        { walletProbeStatus: walletProbeResult.status, walletProbeBody: walletProbeResult.body || "" },
      );
      antiPatterns = antiPatterns.concat(v10Issues);
    } catch (e) {
      console.warn(`  v10 checks eval error on ${route.label}: ${e.message}`);
    }
...
async function main() {
  ...
  for (const route of ROUTES) {
    ...
    const result = await visitRoute(ctx, route, walletProbeResult);
    results.push(result);
