...
export const AEQI_COMPANY_PROGRAM_ID = new PublicKey(
 ...
 * blob under in the company's BytesConfig slot. Mirrors
...
/** `seeds = [b"company", company_id]` against `AEQI_COMPANY_PROGRAM_ID`. */
export function deriveCompanyPda(companyId: Uint8Array): PublicKey {
  if (companyId.length !== 32) {
    throw new Error(`companyId must be exactly 32 bytes (got ${companyId.length})`);
  }
  return findPda([utf8("company"), companyId], AEQI_COMPANY_PROGRAM_ID);
}

/** `seeds = [b"module", company_pda, module_id]` against `AEQI_COMPANY_PROGRAM_ID`. */
export function deriveModulePda(trustPda: PublicKey, moduleId: Uint8Array): PublicKey {
  ...
  return findPda([utf8("module"), trustPda.toBytes(), moduleId], AEQI_COMPANY_PROGRAM_ID);
}

/** `seeds = [b"role_module", company_pda]` against `AEQI_ROLE_PROGRAM_ID`. */
...
/** `seeds = [b"token_module", company_pda]` against `AEQI_TOKEN_PROGRAM_ID`. */
...
/** `seeds = [b"gov_module", company_pda]` against `AEQI_GOVERNANCE_PROGRAM_ID`. */
...
/**
 * `seeds = [b"treasury_module", company_pda]` against `AEQI_TREASURY_PROGRAM_ID`.
...
/**
 * `seeds = [b"treasury_vault_authority", company_pda]` against
...
/** `seeds = [b"unifutures_module", company_pda]` against `AEQI_UNIFUTURES_PROGRAM_ID`. */
...
/** `seeds = [b"mint", company_pda]` against `AEQI_TOKEN_PROGRAM_ID`. */
...
/** `seeds = [b"token_authority", company_pda]` against `AEQI_TOKEN_PROGRAM_ID`. */
...
/**
 * `seeds = [b"curve", company_pda, GENESIS_CURVE_ID]` against
...
/**
 * `seeds = [b"curve_authority", company_pda, GENESIS_CURVE_ID]` against
 * `AEQI_UNIFUTURES_PROGRAM_ID`.
 */
 ...
 * `TOKEN_CONFIG_KEY`. Owned by `aeqi_company`; `aeqi_token::finalize` reads
