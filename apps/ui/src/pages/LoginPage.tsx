...
export default function LoginPage() {
  ...
  const [step, setStep] = useState<"credentials" | "verify" | "2fa" | "totp" | "forgot">("credentials");
      ...
      <div className="auth-container" role="region" aria-live="polite">
        ...
        {step === "credentials" && (
          <>
            <form className="auth-form" onSubmit={handleCredentialsSubmit} autoComplete="on">
              <input className="auth-input" type="email" name="email" placeholder="Email address" aria-label="Email address" autoComplete="username" value={email} onChange={(e) => { setEmail(e.target.value); clearError(); }} autoFocus />
              <PasswordInput placeholder="Password" autoComplete="current-password" value={password} onChange={(e) => { setPassword(e.target.value); clearError(); }} hasError={!!error} errorId="auth-error" />
              {error && <div className="auth-error" role="alert" id="auth-error">{error}</div>}
              <button className="auth-btn-primary" type="submit" disabled={loading || !email.trim() || !password}>
        ...
        {step === "2fa" && (
          <>
            ...
            <p className="auth-switch">
              <a href="#" onClick={(e) => { e.preventDefault(); setStep("credentials"); setTwoFaCode(["", "", "", "", "", ""]); setTwoFaError(""); }}>Back to login</a>
            </p>
          </>
        )}
        ...
        {step === "totp" && (
          <>
            <div className="verify-code-inputs" onPaste={(e) => {
              e.preventDefault();
            ...
            {twoFaError && <div className="auth-error" role="alert">{twoFaError}</div>}
            {twoFaLoading && <p style={{ textAlign: "center", fontSize: 13, color: "var(--text-muted)" }}>Verifying...</p>}
        ...
        {step === "forgot" && (
          <>
            ...
            <p className="auth-switch">
              <a href="#" onClick={(e) => { e.preventDefault(); setStep("credentials"); setForgotSent(false); setForgotError(""); }}>Back to login</a>
            </p>
          </>
        )}
