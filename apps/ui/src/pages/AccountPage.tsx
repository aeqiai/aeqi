...
const CheckIcon = () => <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="var(--success)" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><polyline points="3.5 8.5 6.5 11.5 12.5 5.5" /></svg>;
const GoogleIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden="true"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg>;
const GitHubIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.009-.866-.013-1.7-2.782.604-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z" /></svg>;
...
export default function AccountPage() {
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      setProfileFeedback({ type: "error", msg: e instanceof Error ? e.message : "Failed to update profile." });
    } finally { setProfileSaving(false); }
  };

  const handlePhishingSave = async () => {
    setPhishingFeedback(null);
    if (phishingCode.length < 3 || phishingCode.length > 100) { setPhishingFeedback({ type: "error", msg: "Phrase must be 3-100 characters." }); return; }
    ...
    try {
      ...
      setPhishingFeedback({ type: "error", msg: e instanceof Error ? e.message : "Failed to update security phrase." });
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 8) { setPasswordFeedback({ type: "error", msg: "Password must be at least 8 characters." }); return; }
    ...
    try {
      ...
      setPasswordFeedback({ type: "error", msg: e instanceof Error ? e.message : "Password change failed." });
    ...
    <>
      ...
      <div className="account-page">
          ...
          <>
            <div className="account-profile-header">
              <label className="account-avatar-label" aria-label="Change avatar">
                ...
                <input type="file" accept="image/*" className="account-hidden-input" onChange={async (e) => {
                  const file = e.target.files?.[0];
                  if (!file) return;
                  if (file.size > 2 * 1024 * 1024) { setProfileFeedback({ type: "error", msg: "Image must be under 2 MB." }); return; }
            ...
            <div className="account-name-grid">
              <div>
                <label className="account-field-label" htmlFor="account-first-name">First name</label>
                <input id="account-first-name" className="auth-input" type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First name" autoComplete="given-name" />
              </div>
              <div>
                <label className="account-field-label" htmlFor="account-last-name">Last name</label>
                <input id="account-last-name" className="auth-input" type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Last name" autoComplete="family-name" />
              </div>
            </div>

            <div className="account-field-sm">
              <label className="account-field-label" htmlFor="account-email">Email</label>
              <input id="account-email" className="auth-input account-disabled-input" type="email" value={email} disabled />
            </div>

            <div className="account-field-md">
              <label className="account-field-label" htmlFor="account-phone">Phone</label>
              <input id="account-phone" className="auth-input" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 (555) 000-0000" autoComplete="tel" />
              <p className="account-field-desc">Optional. Used for SMS verification in the future.</p>
            </div>
            ...
            <div className="account-kyc-card">
              <div className="account-kyc-row">
                <div>
                  ...
                  <p className="account-field-desc">Required for incorporation, equity issuance, and marketplace access.</p>
                </div>
            ...
            {profileFeedback && <div className={`account-feedback account-feedback-${profileFeedback.type}`} role="status" aria-live="polite">{profileFeedback.msg}</div>}

            {authMode !== "none" && (
              <>
                <div className="account-divider" />
                <button type="button" className="btn account-sign-out-btn" onClick={() => { logout(); navigate("/login"); }}>Sign out</button>
              </>
            )}
          </>
        )}
          ...
          <>
            ...
            <div className="account-field-lg">
              ...
              {totpEnabled ? (
                <div className="account-totp-status">
                  ...
                  <button type="button" className="btn account-totp-disable-btn" onClick={async () => {
                    ...
                    } catch { setTotpFeedback({ type: "error", msg: "Failed to disable authenticator." }); }
                  }}>Disable</button>
                </div>
              ) : totpSetup ? (
                <div>
                  <p className="account-field-desc">Scan this QR code with your authenticator app, then enter the 6-digit code to verify.</p>
                  <div className="account-qr-container">
                    <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(totpSetup.uri)}`} alt="QR code for authenticator setup" width={200} height={200} />
                  </div>
                  <p className="account-manual-entry">Manual entry: <code>{totpSetup.secret}</code></p>
                  <div className="account-field-row">
                    <input className="auth-input account-totp-input" type="text" inputMode="numeric" maxLength={6} value={totpCode} onChange={(e) => { setTotpCode(e.target.value.replace(/\D/g, "")); setTotpFeedback(null); }} placeholder="6-digit code" aria-label="Authenticator code" />
                    <button type="button" className="btn btn-primary" disabled={totpLoading || totpCode.length !== 6} onClick={async () => {
                      setTotpLoading(true);
                      try {
                      ...
                      } catch { setTotpFeedback({ type: "error", msg: "Invalid code. Try again." }); }
                ...
                <div>
                  <p className="account-field-desc">Add an authenticator app for stronger login security. When enabled, you'll enter an app code instead of an email code.</p>
                  <button type="button" className="btn btn-primary" onClick={async () => {
                    ...
                    } catch { setTotpFeedback({ type: "error", msg: "Failed to start authenticator setup." }); }
                  }}>Set up authenticator</button>
                </div>
              )}
              {totpBackupCodes.length > 0 && (
                <div className="account-backup-codes">
                  <label className="account-field-label account-backup-codes-label">Backup codes -- save these now</label>
                  <p className="account-field-desc">Each code can only be used once. Store them somewhere safe.</p>
                  <div className="account-backup-codes-grid">
                    {totpBackupCodes.map((c) => <code key={c} className="account-backup-code">{c}</code>)}
                  </div>
                </div>
              )}
              {totpFeedback && <div className={`account-feedback account-feedback-${totpFeedback.type}`} role="status" aria-live="polite">{totpFeedback.msg}</div>}
            </div>

            {/* Phishing Code */}
            <div className="account-field-lg">
              <label className="account-field-label" htmlFor="account-phishing-code">Email security phrase</label>
              <p className="account-field-desc">A personal phrase included in every email from AEQI. If the phrase is missing, the email isn't from us.</p>
              <div className="account-field-row">
                <input id="account-phishing-code" className="auth-input" type="text" value={phishingCode} onChange={(e) => { setPhishingCode(e.target.value); setPhishingFeedback(null); }} placeholder="e.g., blue ocean 42" maxLength={100} />
                <button type="button" className="btn btn-primary" onClick={handlePhishingSave} disabled={phishingSaving}>{phishingSaving ? "..." : "Save"}</button>
              </div>
              {phishingFeedback && <div className={`account-feedback account-feedback-${phishingFeedback.type}`} role="status" aria-live="polite">{phishingFeedback.msg}</div>}
            </div>

            {/* Change Password */}
            <div className="account-field-lg">
              <label className="account-field-label">Change password</label>
              <form className="account-password-form" onSubmit={handlePasswordChange} autoComplete="off">
                <div className="auth-password-wrap">
                  <input className="auth-input auth-input-password" type={showCurrentPw ? "text" : "password"} value={currentPassword} onChange={(e) => { setCurrentPassword(e.target.value); setPasswordFeedback(null); }} placeholder="Current password" autoComplete="current-password" aria-label="Current password" />
                  <button type="button" className="auth-password-toggle" onClick={() => setShowCurrentPw(!showCurrentPw)} tabIndex={-1} aria-label={showCurrentPw ? "Hide current password" : "Show current password"}><EyeIcon visible={showCurrentPw} /></button>
                </div>
                <div className="auth-password-wrap">
                  <input className="auth-input auth-input-password" type={showNewPw ? "text" : "password"} value={newPassword} onChange={(e) => { setNewPassword(e.target.value); setPasswordFeedback(null); }} placeholder="New password" autoComplete="new-password" aria-label="New password" />
                  <button type="button" className="auth-password-toggle" onClick={() => setShowNewPw(!showNewPw)} tabIndex={-1} aria-label={showNewPw ? "Hide new password" : "Show new password"}><EyeIcon visible={showNewPw} /></button>
                </div>
                <div className="auth-password-wrap">
                  <input className="auth-input auth-input-password" type={showConfirmPw ? "text" : "password"} value={confirmPassword} onChange={(e) => { setConfirmPassword(e.target.value); setPasswordFeedback(null); }} placeholder="Confirm new password" autoComplete="new-password" aria-label="Confirm new password" />
                  <button type="button" className="auth-password-toggle" onClick={() => setShowConfirmPw(!showConfirmPw)} tabIndex={-1} aria-label={showConfirmPw ? "Hide confirm password" : "Show confirm password"}><EyeIcon visible={showConfirmPw} /></button>
                </div>
                {passwordFeedback && <div className={`account-feedback account-feedback-${passwordFeedback.type}`} role="status" aria-live="polite">{passwordFeedback.msg}</div>}
            ...
            <div className="account-field-lg">
              ...
              <div className="account-connected-list">
                {provider === "google" ? (
                  <div className="account-connected-item account-connected-active"><GoogleIcon /><span>Connected with Google</span><CheckIcon /></div>
                ) : (
                  <button type="button" className="btn account-connect-btn" onClick={() => { window.location.href = "/api/auth/google"; }}><GoogleIcon /> Connect Google</button>
                )}
                {provider === "github" ? (
                  <div className="account-connected-item account-connected-active"><GitHubIcon /><span>Connected with GitHub</span><CheckIcon /></div>
                ) : (
                  <button type="button" className="btn account-connect-btn" onClick={() => { window.location.href = "/api/auth/github"; }}><GitHubIcon /> Connect GitHub</button>
                )}
              </div>
            </div>
            ...
            <div className="account-danger-zone">
              ...
              <p className="account-field-desc">Permanently delete your account and all associated data. This cannot be undone.</p>
              <button
                type="button"
            ...
            <div className="account-activity-section">
              ...
              <p className="account-field-desc account-activity-desc">Recent security events on your account.</p>
              {activity.length === 0 ? (
                  ...
                  {activity.slice(0, 20).map((event, i) => {
                    ...
                    return (
                      <div key={i} className="account-activity-item">
                        <div className={`account-activity-dot ${isError ? "account-activity-dot--error" : "account-activity-dot--success"}`} aria-hidden="true" />
                        <div className="account-activity-body">
                          <span className="account-activity-action">{event.action.replace(/_/g, " ")}</span>
                          {event.detail && <span className="account-activity-detail">{event.detail}</span>}
                        </div>
          ...
          <>
            ...
            {inviteCodes.length === 0 ? (
              ...
              <div className="account-invites-list">
                {inviteCodes.map((inv) => (
                  <div key={inv.code} className={`account-invite-item ${inv.used ? "account-invite-item--used" : "account-invite-item--available"}`}>
                    <code className={`account-invite-code ${inv.used ? "account-invite-code--used" : "account-invite-code--available"}`}>{inv.code}</code>
                    {inv.used ? (
        ...
        {activeTab === "preferences" && (
          <>
            <p className="account-field-desc account-prefs-desc">Transactional emails (login codes, password resets) are always sent.</p>
