...
const OCCUPANT_RANK: Record<string, number> = { agent: 0, human: 1, company: 2, vacant: 3 };
...
/**
 * Roles — the company's authority graph.
 *
 * Canvas composition:
 ...
 */
export default function CompanyRolesTab({ companyId }: { companyId: string }) {
  ...
  useEffect(() => {
    ...
    params.delete("role");
    navigate(entityPathFromId(entities, companyId, "roles", legacySelectedRoleId), {
      replace: true,
      state: {
        rolesReturnTo: `${location.pathname}${params.toString() ? `?${params.toString()}` : ""}`,
      },
    });
  }, [entities, legacySelectedRoleId, location.pathname, navigate, searchParams, companyId]);
  ...
  const browseRoleTemplates = useCallback(() => {
    navigate(`/templates?kind=roles&import_into=${encodeURIComponent(companyId)}`);
  }, [navigate, companyId]);
  ...
  const openRoleTemplate = useCallback(
    (template: SingleBlueprint) => {
      ...
      navigate(
        `/templates/${encodeURIComponent(id)}?kind=roles&import_into=${encodeURIComponent(companyId)}`,
      );
    },
    [browseRoleTemplates, navigate, companyId],
  );
  ...
  const newRolePath = useMemo(() => {
    const base = entityPathFromId(entities, companyId, "roles");
  ...
  }, [entities, searchParams, companyId]);

  const handleSelectRole = useCallback(
    (role: Role) => {
      navigate(entityPathFromId(entities, companyId, "roles", role.id), {
        state: { rolesReturnTo: `${location.pathname}${location.search}` },
      });
    },
    [entities, location.pathname, location.search, navigate, companyId],
  );

  const handleRoleCreated = useCallback(
    async (role: Role) => {
      const refreshed = await api.getRoles(companyId);
    ...
    [entities, location.pathname, navigate, searchParams, companyId],
  );
  ...
  return (
    <div className="company-roles company-primitive-shell">
      <PrimitivePageHeader
        className="company-roles-page-header company-primitive-shell-header"
        title={
          <span className="company-primitive-page-title">
            <span className="company-primitive-page-title-text">Roles</span>
        ...
        actions={
          <div className="company-roles-header-actions">
            <Button
      ...
      >
        <div className="ideas-toolbar company-roles-toolbar">
      ...
      <div
        className={`company-roles-main company-roles-main--detail-collapsed company-roles-main--${view} company-primitive-shell-surface`}
      >
        <div className="company-roles-workspace">
          <section
            className={`company-roles-content company-roles-content--${view}`}
            aria-label="Role workspace"
          >
            <div className={`company-roles-canvas company-roles-canvas--${view}`}>
              ...
              {!loading && !error && filtered.length > 0 && view === "cards" && (
                <div className="company-roles-scroll company-roles-scroll--cards">
                  <RolesCards
                    roles={filtered}
                    agentNames={agentNames}
                    agentAvatars={agentAvatars}
              ...
              {!loading && !error && filtered.length > 0 && view === "list" && (
                <div className="company-roles-scroll company-roles-scroll--list">
                  <RolesList
                    roles={filtered}
                    edges={edges}
                    allRoles={roles}
...
function SuggestedRoleTemplates({
  ...
  return (
    <section className="company-roles-suggest" aria-label="Role templates">
      <header className="company-roles-suggest-head">
        <div className="company-roles-suggest-titles">
          <div className="company-roles-suggest-title-row">
            <h2 className="company-roles-suggest-title">Role templates</h2>
            <span className="company-roles-suggest-count" aria-hidden>
              {templates.length}
            </span>
          </div>
        </div>
        <Button
          ...
          variant="secondary"
          size="sm"
          className="company-roles-suggest-all"
          onClick={onBrowse}
      ...
      {loading ? (
        <div className="company-roles-suggest-state">
          <Loading size="sm" /> Loading role templates…
        </div>
      ) : error ? (
        <div className="company-roles-suggest-state" role="status">
        ...
        <div className="company-roles-suggest-grid">
          {visible.map((template) => (
            <button
              key={blueprintId(template)}
              type="button"
              className="company-roles-suggest-card"
              onClick={() => onOpenTemplate(template)}
              aria-label={`View ${template.name} role template`}
            >
              <h3 className="company-roles-suggest-card-title">{template.name}</h3>
              <p className="company-roles-suggest-card-desc">
                {template.tagline || roleTemplateStructureLine(template)}
              </p>
              <p className="company-roles-suggest-card-meta">{roleTemplateRuntimeLine(template)}</p>
              <span className="company-roles-suggest-card-cta" aria-hidden>
                View template
...
function RolesLoading() {
  return (
    <div className="company-roles-state">
...
function RolesError({ message }: { message: string }) {
  return <div className="company-roles-state company-roles-state--error">{message}</div>;
}
