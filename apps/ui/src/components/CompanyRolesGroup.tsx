...
interface CompanyRolesGroupProps {
  companyId: string;
  basePath: string;
}
 ...
 * under the company hero, sitting above the Execution row. Parallels
 * the Execution / Ownership group components so the company overview
 ...
 */
export default function CompanyRolesGroup({ companyId, basePath }: CompanyRolesGroupProps) {
  ...
  return (
    <section
      className="company-cockpit-card company-cockpit-card--roles"
      aria-labelledby="company-roles-heading"
    >
      <header className="company-cockpit-card-header">
        <h2 id="company-roles-heading" className="company-cockpit-card-title">
          Roles
        </h2>
        <span className="company-cockpit-card-sub">Authority</span>
      </header>
      <div className="company-cockpit-inner-grid">
        <PrimitiveCard
          to={`${basePath}/roles`}
          ...
          footer={
            total > 0 ? (
              <span className="company-quest-signals" aria-label="role composition breakdown">
                <span className="company-quest-signal" title="Directors">
                  <span className="company-role-dot company-role-dot--director" aria-hidden />
                  {directors}
                </span>
                <span className="company-quest-signal" title="Operators">
                  <span className="company-role-dot company-role-dot--operator" aria-hidden />
                  {operators}
                </span>
                <span className="company-quest-signal" title="Vacant">
                  <span className="company-role-dot company-role-dot--vacant" aria-hidden />
                  {vacant}
                </span>
              </span>
            ) : undefined
...
function PrimitiveCard({ to, icon, label, value, hint, sub, tone, footer }: PrimitiveCardProps) {
  return (
    <Link
      to={to}
      className={`company-cockpit-mini${tone === "warmth" ? " company-cockpit-mini--warmth" : ""}`}
    >
