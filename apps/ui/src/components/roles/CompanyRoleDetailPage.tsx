...
export default function CompanyRoleDetailPage({
  ...
  }, [handleRoleUpdated, role, role?.description_idea_id, role?.id, role?.title, companyId]);
  ...
  return (
    <div
      className={`company-roles company-role-detail-page${
        detailsCollapsed ? " company-role-detail-page--details-collapsed" : ""
      }`}
    >
      <PrimitivePageHeader
        className="company-roles-page-header company-role-detail-page-header"
        title={
          <Button
            type="button"
            variant="secondary"
            size="sm"
            className="company-role-detail-back"
            ...
            {role && (
              <Tooltip content="Copy role route" portal>
                <IconButton
                  variant="bordered"
                  size="md"
                  className="company-roles-header-icon"
      ...
      <div className="company-roles-main company-roles-main--detail-collapsed">
        <div className="company-roles-workspace company-role-detail-workspace">
          <section className="company-roles-content" aria-label="Role detail workspace">
            <div className="company-role-detail-canvas">
              {loading && (
              ...
              {error && (
                <div className="company-roles-state company-roles-state--error">{error}</div>
              )}
              {!loading && !error && !role && (
                <div className="company-roles-state">
                  <EmptyState
                    title="Role not found"
                    description="This role is no longer available in the company authority graph."
                    action={
              ...
              {!loading && !error && role && (
                <>
                  <RoleDetailExplorer
                    ...
                    onOpenRole={openRole}
                  />
                  <main className="company-role-detail-document" aria-label="Role idea">
                    {ideaLoading && (
                      <div className="company-roles-state">
                    ...
                    {ideaError && (
                      <div className="company-roles-state company-roles-state--error">
                        {ideaError}
                      </div>
                    )}
                    {!ideaLoading && !ideaError && roleIdea && (
                      <IdeaCanvas
                        ref={canvasRef}
                        agentId={roleIdea.agent_id ?? companyId}
                        idea={roleIdea}
                        ...
                        hideMetaStrip
                        contentHeaderSlot={
                          <div className="company-role-detail-surface-header">
                            <div className="company-role-detail-surface-title">Role idea</div>
                            <Tooltip
                            ...
                            >
                              <button
                                type="button"
                                className="role-inspector-icon-action company-role-detail-surface-action"
                                aria-label={
                  ...
                  {!detailsCollapsed && (
                    <aside className="company-role-detail-inspector" aria-label="Role details">
                      <RoleInspector
                        role={role}
                        edges={edges}
                        rolesById={rolesById}
