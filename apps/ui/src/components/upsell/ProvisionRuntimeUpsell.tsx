 ...
 * Sessions) on a free COMPANY.
 *
 * Architecture:
 *   - Free Companies have an on-chain `company_address` and an empty
 ...
 *   - Wire: `POST /api/runtime/provision { company_id, plan }` returns a
 *     Stripe checkout URL; the `customer.subscription.created` webhook
 *     calls `upgrade_signup_company_to_runtime` to spin the service.
 *
...
export interface ProvisionRuntimeUpsellProps {
  ...
  /** Platform-side entity uuid (matches `Company.id`). */
  companyId: string;
}
...
export function ProvisionRuntimeUpsell({ surface, companyId }: ProvisionRuntimeUpsellProps) {
      ...
      <Card variant="default" padding="lg" className="runtime-upsell-card">
        <header className="runtime-upsell-header">
          ...
          <p className="runtime-upsell-lede">
            Your COMPANY is on-chain. A runtime adds agents that act on its behalf.
          </p>
        </header>
