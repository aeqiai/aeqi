...
interface TrustGraphZoomViewportProps {
  children: ReactNode;
  width: number;
  height: number;
}

export default function TrustGraphZoomViewport({
  children,
  width,
  height,
}: TrustGraphZoomViewportProps) {
  ...
  const onPointerDown = useCallback((event: PointerEvent<HTMLDivElement>) => {
    ...
    if (target.closest(".trust-context-map-node")) return;
    event.preventDefault();
  ...
  return (
    ...
    >
      <div
        ref={innerRef}
        className="trust-context-map-inner"
        style={{
      ...
      <div className="trust-context-map-controls" role="toolbar" aria-label="Zoom controls">
        <IconButton
          aria-label="Zoom in"
          variant="bordered"
          size="xs"
