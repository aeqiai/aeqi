...
import type { ActivityEntry, Agent, EntityViewWidgetKind, Quest, Company } from "@/lib/types";
...
export const WIDGETS: WidgetDefinition[] = [
  {
    kind: "identity",
    label: "Identity",
    description: "COMPANY status, public surface, and on-chain identity.",
    source: "company",
    icon: Globe2,
  },
  {
  ...
  },
  {
    kind: "agents",
    label: "Agents",
    description: "Runtime workers assigned to this COMPANY.",
    source: "agents",
    icon: Bot,
  },
  {
...
export function ViewWidget({ kind, data }: { kind: EntityViewWidgetKind; data: WidgetRenderData }) {
  ...
  return (
    <article className="company-view-widget">
      <header className="company-view-widget-head">
        <span className="company-view-widget-icon">
          <Icon size={16} strokeWidth={1.7} aria-hidden />
        </span>
        <span className="company-view-widget-copy">
          <strong>{definition.label}</strong>
          <small>{definition.description}</small>
        </span>
      </header>
      <div className="company-view-widget-metric">
        <span>{details.value}</span>
        <small>{details.label}</small>
      </div>
      <ul className="company-view-widget-list" aria-label={`${definition.label} signals`}>
...
function widgetDetails(kind: EntityViewWidgetKind, data: WidgetRenderData) {
  ...
  const hasChain = Boolean(data.entity?.company_address || data.entity?.company_id);
  ...
  switch (kind) {
    case "identity":
      return {
        ...
        rows: [
          data.entity?.name ?? "Unknown COMPANY",
          data.entity?.slug ? `${data.entity.slug}.aeqi.ai` : "No public slug",
          hasChain ? "COMPANY address present" : "COMPANY address pending",
        ],
      };
    case "sessions":
      return {
    ...
    case "economy":
      return {
        ...
        rows: ["Markets verify indexed COMPANY identity", "Cap-table seed status lives in Markets"],
      };
    case "website":
      return {
