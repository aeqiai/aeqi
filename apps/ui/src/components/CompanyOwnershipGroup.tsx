...
interface CompanyOwnershipGroupProps {
 ...
 * · Incorporation) under the company hero. The header bar (COMPANY address
 * + signers + on-chain mirror status) that lived here in v3 was lifted
 * into the hero card's right-side overview panel (CompanyHeroOverview)
 ...
 */
export default function CompanyOwnershipGroup({
  companyAddress,
  basePath,
}: CompanyOwnershipGroupProps) {
  const assets = useAssets(companyAddress);
  const equity = useEquity(companyAddress);
  const quorum = useQuorum(companyAddress);
  ...
  // "Provisioning" = platform-side COMPANY registered but the matching
  // Solana module hasn't materialized yet (no companyAddress, or company
  ...
  return (
    <section
      className="company-cockpit-card company-cockpit-card--ownership"
      aria-labelledby="company-ownership-heading"
    >
      <header className="company-cockpit-card-header">
        <h2 id="company-ownership-heading" className="company-cockpit-card-title">
          Capital &amp; Governance
        </h2>
        <span className="company-cockpit-card-sub">Solana state</span>
      </header>
...
function OwnershipPrimitiveCard({
  ...
  const body = (
    <>
      ...
      {comingSoon ? (
        ...
        <span className="company-primitive-pending company-primitive-pending--graphite">v2</span>
      ) : provisioning ? (
        ...
        <span className="company-primitive-pending" role="status">
  ...
  if (comingSoon || !to) {
    return (
      <div
        className="company-cockpit-mini company-cockpit-mini--soon"
        aria-disabled="true"
