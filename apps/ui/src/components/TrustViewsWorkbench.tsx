...
} from "@/components/TrustViewsWorkbench.widgets";
...
interface StoredViewsState {
  ...
  views?: TrustDashboardView[];
}

interface TrustViewsWorkbenchProps {
  trustId: string;
  editable?: boolean;
}

const STORAGE_KEY = "aeqi_trust_views_v1";

const DEFAULT_VIEWS: TrustDashboardView[] = [
  {
    id: "overview",
    title: "Overview",
    scope: "public",
...
function cloneDefaultViews(): TrustDashboardView[] {
  return DEFAULT_VIEWS.map((view) => ({ ...view, widgets: [...view.widgets] }));
}

function normalizeViews(candidate: unknown): TrustDashboardView[] | null {
  if (!Array.isArray(candidate)) return null;
  const knownKinds = new Set(WIDGETS.map((widget) => widget.kind));
  const views = candidate.flatMap((item): TrustDashboardView[] => {
    if (!item || typeof item !== "object") return [];
    const record = item as Partial<TrustDashboardView>;
...
function normalizeApiViews(apiViews: EntityView[]): TrustDashboardView[] | null {
  const knownKinds = new Set(WIDGETS.map((widget) => widget.kind));
  const views = apiViews.flatMap((view): TrustDashboardView[] => {
...
function toEntityViewUpserts(views: TrustDashboardView[]): EntityViewUpsert[] {
...
function readStoredState(trustId: string): StoredViewsState | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    const trustState = parsed?.[trustId];
...
function persistState(trustId: string, state: StoredViewsState) {
...
function createInitialState(trustId: string): Required<StoredViewsState> {
  const stored = readStoredState(trustId);
...
export default function TrustViewsWorkbench({
  trustId,
  editable = false,
}: TrustViewsWorkbenchProps) {
  const entities = useDaemonStore((s) => s.entities);
  const agents = useDaemonStore((s) => s.agents);
  const quests = useDaemonStore((s) => s.quests);
  const events = useDaemonStore((s) => s.events);
  const entity = entities.find((candidate) => candidate.id === trustId);
  const basePath = entity ? entityBasePath(entity) : `/trust/${encodeURIComponent(trustId)}`;
  const trustAgents = useMemo(
    () => agents.filter((agent) => agent.trust_id === trustId || agent.trust_id === entity?.id),
    [agents, entity?.id, trustId],
  );
  ...
  const trustEvents = useMemo(() => {
    ...
    const trustRoots = new Set(
      [trustId, entity?.id, entity?.slug, entity?.trust_address, entity?.trust_id].filter(
  ...
  const [views, setViews] = useState<TrustDashboardView[]>(() => createInitialState(trustId).views);
  const [selectedViewId, setSelectedViewId] = useState(
    () => createInitialState(trustId).selectedViewId,
  );

  const persistViewsToApi = (nextViews: TrustDashboardView[]) => {
    if (!editable) return;
    void api
      .upsertTrustViews(trustId, toEntityViewUpserts(nextViews))
      .then((response) => {
        const normalized = normalizeApiViews(response.views ?? []);
        if (!normalized) return;
        setViews(normalized);
        persistState(trustId, { selectedViewId, views: normalized });
      })
      .catch(() => {
  ...
  useEffect(() => {
    const next = createInitialState(trustId);
    setViews(next.views);
  ...
  useEffect(() => {
    ...
    void api
      ...
      .then((response) => {
        ...
        persistState(trustId, { selectedViewId: nextSelected, views: apiViews });
      })
      .catch(() => {
  ...
  useEffect(() => {
    persistState(trustId, { selectedViewId, views });
  }, [selectedViewId, trustId, views]);
  ...
  const addView = () => {
    ...
    const nextView: TrustDashboardView = {
  ...
  const updateSelectedView = (updater: (view: TrustDashboardView) => TrustDashboardView) => {
  ...
  return (
    <section
      className="trust-views-workbench"
      aria-label={editable ? "Views dashboard builder" : "TRUST overview"}
    >
      <div className="trust-views-top">
        <div className="trust-views-heading">
          <span className="trust-views-kicker">Views</span>
        ...
        <div className="trust-views-actions">
          <span
            className="trust-views-scope"
          ...
          {editable && (
            <Button
              ...
              size="sm"
              variant="secondary"
              className="trust-views-new-button"
      ...
      <div className="trust-views-tabs" role="tablist" aria-label="Saved views">
        {views.map((view) => (
          <button
            key={view.id}
            type="button"
            ...
            className={
              view.id === selectedView.id ? "trust-views-tab is-active" : "trust-views-tab"
            }
      ...
      <div className={editable ? "trust-views-editor" : "trust-views-editor is-readonly"}>
        <div className="trust-views-canvas" aria-label={`${selectedView.title} widgets`}>
          {selectedView.widgets.length === 0 ? (
            <div className="trust-views-empty">
              <strong>{editable ? "No widgets selected" : "No overview widgets available"}</strong>
              <span>
                {editable
                  ? "Add widgets from the library to shape this view."
                  : "Launch signals will appear here as the TRUST starts operating."}
              </span>
            </div>
          ) : (
        ...
        {editable && (
          <aside className="trust-views-library" aria-label="Available widgets">
            <div className="trust-views-library-head">
              <strong>Widget library</strong>
              <span>{selectedView.widgets.length} active</span>
            </div>
            <div className="trust-views-scope-toggle" aria-label="View visibility">
              <button
                type="button"
              ...
              {WIDGETS.map((widget) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={
                      active ? "trust-views-widget-pick is-active" : "trust-views-widget-pick"
                    }
                    aria-pressed={active}
