/**
 * `useCompanyHealthMetrics(addr, windowDays)` — operator day-30 health.
 *
 * Computes four substrate-compound metrics for a COMPANY, plus 30d
 ...
 *     today (cf. `CompanyOverviewTab.tsx`).
 ...
 *   - Adding a `/api/company/<addr>/health-metrics` endpoint would push the
...
export interface HealthMetrics {
   ...
   *  28d divided by closed quests in the same period. Company health
  ...
  /** Earliest known activity timestamp (epoch ms) for the COMPANY — drives
...
export interface UseCompanyHealthMetricsResult {
  metrics: HealthMetrics | null;
  isLoading: boolean;
  error: Error | null;
}

export interface CompanyHealthMetricsOptions {
  /** Optional per-agent filter for `/company/<addr>/agents/<agent>/health`. */
  agentId?: string | null;
}
 ...
 *
 * `addr` is the canonical /company/<addr> identifier. We resolve it to
 * an entity via the daemon store's entity list so the function caller
 * can use either the raw URL slug or a resolved company_address.
 */
export function useCompanyHealthMetrics(
  addr: string | null | undefined,
  windowDays: number = DEFAULT_HEALTH_WINDOW_DAYS,
  options: CompanyHealthMetricsOptions = {},
): UseCompanyHealthMetricsResult {
  ...
  // Resolve addr → entity. Match by company_address first (canonical),
  // then by id as a fallback for callers that pass an entity id.
  const entity = useMemo(() => {
    if (!addr) return null;
    return (
      entities.find((e) => e.company_address === addr) ??
      entities.find((e) => e.id === addr) ??
      null
    );
  }, [addr, entities]);

  const companyId = entity?.id ?? null;

  // Subtree agents = root + descendants (every agent whose company_id is
  // this entity, plus the entity-as-agent itself when present). Mirrors
  // CompanyOverviewTab's subtree derivation so the metric scope agrees
  // with the cockpit numbers.
  const subtreeAgents = useMemo(() => {
    if (!companyId) return [];
    return allAgents.filter((a) => a.company_id === companyId || a.id === companyId);
  }, [allAgents, companyId]);
  ...
  }, [companyId, isLoading, windowDays, allQuests, mergedEvents, ideas, agentNames, agentIds]);
