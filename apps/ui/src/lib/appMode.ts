...
function resolveCompanyAddress(companyAddress: string): string | null {
  try {
    const raw = localStorage.getItem("aeqi_daemon_entities");
    if (!raw) return null;
    const entities = JSON.parse(raw) as Array<{ id?: string; company_address?: string | null }>;
    if (!Array.isArray(entities)) return null;

    const entity = entities.find((item) => item?.company_address === companyAddress) ?? null;
    if (entity?.id) return entity.id;

    const sameId = entities.find((item) => item?.id === companyAddress) ?? null;
...
/**
 * Resolve the routing key (`X-Company` header / WS `?root=`) for the current
 * URL. The canonical shell is `/company/:companyAddress/...`; user-scope routes
 * (`/account`, `/launch`, `/sessions/:id`, …) return "" so the caller falls
 * back to the cached active entity.
 *
 * The route slug is the on-chain COMPANY address. The backend usually wants the
...
export function getScopedEntity(): string {
  ...
  if (match) {
    const companyAddress = decodeURIComponent(match[1]);
    return resolveCompanyAddress(companyAddress) ?? companyAddress;
  }
