...
async fn platform_get_json(
    ...
    if let Some(company_id) = company_id {
        request = request.header("x-entity", company_id);
    }
...
fn websocket_url(api_url: &str, company_id: &str) -> String {
    ...
    format!(
        "{base}/api/chat/stream?company_id={}",
...
async fn run_remote_chat(
    ...
    let entities = entities_json
        ...
        .cloned()
        .unwrap_or_default();
    let company_id = if let Some(entity) = entity {
        entity.to_string()
    } else {
        ...
        selected
            .get("id")
            .or_else(|| selected.get("company_id"))
            .and_then(|v| v.as_str())
    ...
    let roles_path = format!("/api/roles?company_id={}", urlencoding::encode(&company_id));
    let roles_json = platform_get_json(&client, &api_url, &api_key, &roles_path, Some(&company_id))
        .await
    ...
    let agents_json = platform_get_json(
        &client,
        &api_url,
        &api_key,
        "/api/agents",
    ...
    let ws_handle = Some(spawn_ws_thread(
        websocket_url(&api_url, &company_id),
        vec![("authorization".to_string(), format!("Bearer {api_key}"))],
        cmd_rx,
        event_tx,
    ));
    eprintln!("  \x1b[90m(connected to {api_url}, company {company_id})\x1b[0m");
