...
fn cmd_graph_audit(config_path: &Option<PathBuf>) -> Result<()> {
    ...
    std::fs::create_dir_all(&graph_dir).ok();
    let target_projects: Vec<String> = if config.agent_spawns.is_empty() {
    ...
    if target_projects.is_empty() {
        eprintln!("No graph DBs found in {}", graph_dir.display());
        return Ok(());
    }
...
fn discover_graph_projects(graph_dir: &PathBuf) -> Vec<String> {
