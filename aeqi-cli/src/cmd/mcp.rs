...
pub fn cmd_mcp(config_path: &Option<PathBuf>) -> Result<()> {
    ...
    let tools = vec![
        ...
        ToolDef {
            ...
            description: "Persistent knowledge store. Search, store, update, delete, or give feedback on ideas. The search pipeline is tag-routed BM25+vector with MMR diversification, graph boosts, bi-temporal filtering, and hotness/feedback boosts. Results are served from a daemon-side recall cache that invalidates on writes, so MCP calls stay coherent across tools.".to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["store", "search", "update", "delete", "feedback"],
                        "description": "store: save knowledge (needs name, content, tags). search: find ideas by natural language query (needs query). update: modify an idea by ID (needs id plus name/content/tags). delete: remove an idea by ID (needs id). feedback: mark an idea as used/useful/ignored/wrong/corrected/pinned (needs id and signal)."
                    },
                ...
                let result = match tool_name {
                    ...
                    "ideas" => {
                        ...
                        match action {
                            ...
                            _ => Err(anyhow::anyhow!(
                                "unknown ideas action: {action}. Use: store, search, update, delete, feedback"
                            )),
                        }
                    }
