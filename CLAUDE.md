...
**Incremental cargo build skips files when two commits land back-to-back via the webhook.** When you push `commit A` (e.g. Phase-2 feature), the webhook fires `deploy.sh`, but before it pulls latest you push `commit B` (e.g. follow-up fix to the same file). The webhook's `git pull --ff-only` brings in both, but `cargo build --release -p aeqi --bin aeqi` may incrementally skip recompiling `aeqi-orchestrator` — the resulting binary contains commit A's changes but NOT commit B's. Diagnostic: `strings /home/claudedev/aeqi-platform/runtime/bin/aeqi | grep "<known-string-from-B>"` returns nothing; source on main has the string. Fix: manual `cd /home/claudedev/aeqi && cargo build --release -p aeqi --bin aeqi` (forces full dep walk, ~3-4 min cold), then swap binary via `systemctl stop aeqi-host-<id> && rm /home/claudedev/aeqi-platform/runtime/bin/aeqi && cp target/release/aeqi /home/claudedev/aeqi-platform/runtime/bin/aeqi && systemctl start aeqi-host-<id>` (rm-then-cp avoids `Text file busy` while the host process holds the inode). Cost (2026-05-07): ~6 min on architect Phase-2 ship cycle when d7a2ba11 (substrate-fallback) didn't compile into the binary even though main carried both commits.

**Per-tenant host runtime services do NOT load `/etc/aeqi/secrets.env` — only `aeqi-platform.service` does.** Means `std::env::var("OPENROUTER_API_KEY")` (and any other key shipped in that file) returns `None` in `aeqi-host-<entity_id>.service` processes. Code that needs an LLM key in the runtime must fall back to `aeqi_core::credentials::read_global_legacy_blob_sync(&data_dir, "OPENROUTER_API_KEY")` — the orchestrator's own startup migration already parks decrypted plaintext keys in `~/.aeqi/aeqi.db`'s credentials table. Pattern: the architect IPC handler's `ensure_llm_env_resolved()` helper reads the substrate when both `DEEPINFRA_API_KEY` and `OPENROUTER_API_KEY` are absent in `std::env`, then `unsafe { std::env::set_var(...) }`'s the result so `build_default_llm()` finds it. Idempotent. New IPC handlers fronting LLM/auth providers should follow the same pattern. Cost (2026-05-07): one extra worktree + ship cycle when Phase-2 architect.draft always fell through to the stub generator on host runtimes despite the key being parked in the substrate.
...
**`aeqi-paymaster` has its own deploy lifecycle — `deploy.sh` does NOT touch it.** The `aeqi-paymaster` binary (`/usr/local/bin/aeqi-paymaster`) is a standalone service separate from the main aeqi runtime. `./scripts/deploy.sh` rebuilds and restarts the aeqi orchestrator binary only. After a Rust change to `crates/aeqi-paymaster/`, the operator must: (1) `cargo build -p aeqi-paymaster --release`; (2) `sudo systemctl stop aeqi-paymaster`; (3) `sudo cp target/release/aeqi-paymaster /usr/local/bin/aeqi-paymaster`; (4) `sudo systemctl start aeqi-paymaster`. Infrastructure setup (user, dirs, env file) is a one-time `deploy/aeqi-paymaster.service` install step — not automated. On the dev host the service is `inactive` because it hasn't been provisioned; `active` on the platform host. Don't confuse "binary built successfully" with "service updated." Cost (2026-05-06): deploy step after aa-polish ship discovered service inactive on dev host with no installed binary path.
