...
- **Quest workers silently skipped `session:execution_start` and `session:step_start`.** `scheduler.rs::spawn_worker` called `assemble_ideas_for_quest_start` which walks only `session:start` + `session:quest_start`. The interactive-chat path in `session_manager.rs` fired all four patterns, but quest workers went through a different path that didn't. A user who attached an idea to `on_step_start` expecting per-LLM-call injection got zero firings on every quest. Initially deferred for a design pass (filed as `as-011` with two architectural options); design doc `docs/design/as-011-worker-step-context.md` picked EA/SA/WC — fire `session:execution_start` once per worker-run, bump `step_start` fire_count per session, route step ideas through a new `assemble_step_ideas_for_worker` helper and an `AgentWorker::with_step_ideas` setter. Commits `98cea47` (design) + `796ac74` (implementation + 2 regression tests + 4 drive-by clippy fixes).
- **The MCP `agents(action=get)` tool silently dropped its advertised session primer for months.** The tool description promised "full agent profile with assembled ideas (session primer)" but the implementation sent IPC cmd `"assemble_ideas"` that has *no* handler in `daemon.rs`. `ipc_request_sync` returned an error, the `.ok().and_then(...)` fallback silently discarded it, and every MCP response came back without the `context` field while the tool kept claiming it would be there. Caught by code audit, not runtime probe — the MCP server didn't log a warning. Fix: route through the existing `trigger_event` IPC path with `pattern="session:start"` — same read-only assembly used by preflight and the events.trigger action, no `record_fire` so telemetry stays honest. Commit `9951ee3`.
