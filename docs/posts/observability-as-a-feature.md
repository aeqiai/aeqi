...
- **Quest workers silently skip `session:execution_start` and `session:step_start`.** `scheduler.rs::spawn_worker` calls `assemble_ideas_for_quest_start` which walks only `session:start` + `session:quest_start`. The interactive-chat path in `session_manager.rs` fires all four patterns, but quest workers go through a different path that doesn't. A user who attaches an idea to `on_step_start` expecting per-LLM-call injection gets zero firings on every quest. Filed as task `as-011` with two architectural options (`AgentWorker` step-context provider vs. prune the two patterns from the default seed and document the gap). Deferred for a design pass — shipping a half-fix here would have been worse than filing honestly.
