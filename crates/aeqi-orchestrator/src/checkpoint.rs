...
///
/// Checkpoints are captured **externally** by
...
pub struct AgentCheckpoint {
    ...
    /// Working directory the worker was operating in.
    pub worktree_path: Option<String>,
    /// When this checkpoint was captured.
    pub timestamp: DateTime<Utc>,
    /// Session ID (if the worker had one).
    pub session_id: Option<String>,
    /// Free-form progress notes (e.g., from the worker's last output).
...
impl AgentCheckpoint {
    ...
    ///
    /// Runs git commands against the worker's working directory to observe:
    ...
    /// Set the worker name on this checkpoint.
    ...
    ///
    /// Produces a human-readable summary that a new worker can use to understand
    /// what the previous worker observed.
