//! Memory and knowledge IPC handlers.

use std::path::PathBuf;

pub async fn handle_memories(
    ...
    if allowed.is_some() && (mem_project.is_empty() || mem_project == "*") {
        return serde_json::json!({"ok": true, "memories": [], "count": 0});
    }
...
pub async fn handle_memory_profile(
    ...
    if let Ok(conn) = rusqlite::Connection::open(&db_path) {
        ...
        let dynamic_memories = fetch(&["decision", "context", "insight", "procedure"]);
...
pub async fn handle_memory_prefix(
    ctx: &super::CommandContext,
    request: &serde_json::Value,
