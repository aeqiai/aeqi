...
impl Scene {
    async fn new() -> Self {
        ...
        let company_id = agent.company_id.expect("agent must own entity");
...
async fn step_03_special_project_skips_org_chart() {
    ...
    let tree = handle_budget_tree(
        &ctx,
        &json!({"company_id": s.company_id, "caller_user_id": s.ceo_user}),
        &None,
    )
    .await;
...
async fn step_04_atomic_hire_creates_role_budget_allocation() {
    ...
    let edges = ctx
        .role_registry
        .list_edges_for_entity(&s.company_id)
        .await
        .unwrap();
    assert!(
        edges
...
async fn step_07_multi_role_disambiguation() {
    ...
    // same company — IPC writes that resolve caller_role implicitly must
    // demand `as_role_id`.
    ctx.role_registry
