...
pub fn validate_file_path(value: &str) -> Result<(), validator::ValidationError> {
    ...
    #[cfg(windows)]
    {
        if value.len() >= 2 && value.chars().next().unwrap().is_ascii_alphabetic() && value.chars().nth(1) == Some(':') {
...
pub struct CreateQuestRequest {
    ...
    #[validate(custom = "validate_safe_string")]
...
pub struct FileOperationRequest {
    ...
    #[validate(length(max = 10 * 1024 * 1024))] // 10MB max
...
mod tests {
    ...
    async fn test_handler(Json(body): Json<TestRequest>) -> impl IntoResponse {
