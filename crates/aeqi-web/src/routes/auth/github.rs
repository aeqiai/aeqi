...
async fn github_auth_handler(State(state): State<AppState>) -> Response {
    ...
    if state.auth_config.waitlist {
        return (
            ...
            "GitHub OAuth signup requires invite-code binding; use email signup with an invite code",
        )
            .into_response();
    }
...
async fn github_callback_handler(
    ...
    if let Err(e) = ensure_account_wallet(&state, &user.id).await {
        tracing::error!(user_id = %user.id, error = %e, "github oauth account wallet provisioning failed");
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
