...
pub fn secure_resolve_path(base_dir: &Path, requested_path: &str) -> Result<PathBuf> {
    ...
    let canonical_resolved = resolved.canonicalize()
        .map_err(|e| anyhow::anyhow!("failed to canonicalize path {}: {}", resolved.display(), e))?;
...
pub fn secure_resolve_path_for_write(base_dir: &Path, requested_path: &str) -> Result<PathBuf> {
    ...
    let canonical_parent = parent.canonicalize()
        .map_err(|e| anyhow::anyhow!("failed to canonicalize parent directory {}: {}", parent.display(), e))?;
...
pub fn validate_path_string(path: &str) -> Result<()> {
    ...
    #[cfg(unix)]
    {
        if path.len() >= 2 && path.chars().next().unwrap().is_ascii_alphabetic() && path.chars().nth(1) == Some(':') {
