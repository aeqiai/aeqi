...
//! Bearer <token>` is present and that `X-Company` is non-empty. No real JWT
...
//! - Resolve `company_id` from the `sub` claim.
...
/// In-memory stub balance store. Keyed by company_id.
///
...
impl BalanceStore {
    ...
    /// Return the current balance in cents for `company_id`.
    /// Returns `None` if the entity has no record (treated as zero).
    pub fn get(&self, company_id: &str) -> Option<i64> {
        self.inner.lock().unwrap().get(company_id).copied()
    }

    /// Set the balance for `company_id` (used in tests).
    pub fn set(&self, company_id: &str, cents: i64) {
        self.inner
            .lock()
            .unwrap()
            .insert(company_id.to_owned(), cents);
    }
}
    ...
    ResBody: Default + Send + 'static,
{
    ...
    fn call(&mut self, req: Request<ReqBody>) -> Self::Future {
        ...
        let company_id = company_id.unwrap_or_else(|| "unknown".to_owned());
        let balance = self.store.get(&company_id).unwrap_or(0);

        if balance <= 0 {
            tracing::warn!(company_id, "subscription balance exhausted — returning 402");
            return Box::pin(async move {
                let mut res = Response::new(ResBody::default());
        ...
        tracing::debug!(company_id, balance, "subscription check passed");
