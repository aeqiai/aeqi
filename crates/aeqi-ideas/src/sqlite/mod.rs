...
impl SqliteIdeas {
    ...
    ///
    /// `_decay_halflife_days` is retained for source-compat with the CLI
    /// helper but is no longer a store-level field — decay is now per-tag
    /// via [`crate::tag_policy::TagPolicy::decay_halflife_days`].
    pub fn open(path: &Path, _decay_halflife_days: f64) -> Result<Self> {
