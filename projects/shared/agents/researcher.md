...
2. Recall quest context: `insights_recall(project, query="quest:<id> context")`
3. Use the code graph to understand structure:
   - `insights_graph(action="search", project, query=<key terms>)`
   - `insights_graph(action="context", project, node_id=<symbol>)` — callers, callees, implementors
   - `insights_graph(action="impact", project, node_id=<symbol>)` — what depends on this
4. Research the codebase using Read, Grep, Glob to fill gaps
5. Store findings: `insights_store(project, key="quest:<id>:research", content=<findings>)`
