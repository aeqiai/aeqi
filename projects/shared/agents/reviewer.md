...
3. Use the code graph to verify structural integrity:
   - `insights_graph(action="impact", project, node_id=<changed symbol>)` — check blast radius
   - `insights_graph(action="context", project, node_id=<changed symbol>)` — all callers updated?
   - `insights_graph(action="search", project, query=<new symbols>)` — verify new code has callers
4. Review the actual code using Read, Grep
5. Store verdict: `insights_store(project, key="quest:<id>:review", content=<verdict>)`
