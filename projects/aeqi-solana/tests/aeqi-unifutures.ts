...
describe("aeqi_unifutures", () => {
  ...
  before(async () => {
    fakeCompany = await createCompany(provider, trustProgram, "aeqi-unifutures");
    ...
    [poolAuthorityPda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("liquidity_pool_authority"),
        fakeCompany.toBuffer(),
        Buffer.from(poolId),
      ],
      program.programId,
    ...
    [lpMintPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("lp_mint"), fakeCompany.toBuffer(), Buffer.from(poolId)],
      program.programId,
    );
  ...
  it("init rejects a payer that is not the company authority", async () => {
    ...
    const [moduleStatePda] = PublicKey.findProgramAddressSync(
      [Buffer.from("unifutures_module"), company.toBuffer()],
      program.programId,
    );
  ...
  it("create_liquidity_pool initializes the internal pool shell", async () => {
    ...
    expect(pool.company.toBase58()).to.eq(fakeCompany.toBase58());
    expect(Buffer.from(pool.poolId).toString("hex")).to.eq(
      Buffer.from(poolId).toString("hex"),
    );
  ...
  it("add_liquidity seeds the pool and mints LP shares", async () => {
    ...
    const vaultBase = await getAccount(
      provider.connection,
      baseVault,
      undefined,
      TOKEN_2022_PROGRAM_ID,
  ...
  it("swap_exact_in trades in both directions and updates the pool", async () => {
    const baseToQuote = quoteSwapExactIn(
      poolBaseReserve,
      poolQuoteReserve,
      1_000n,
      30n,
    ...
    let baseAccount = await getAccount(
      provider.connection,
      providerBaseAta,
      undefined,
      TOKEN_2022_PROGRAM_ID,
    );
    let quoteAccount = await getAccount(
      provider.connection,
      providerQuoteAta,
      undefined,
      TOKEN_2022_PROGRAM_ID,
    ...
    expect(quoteAccount.amount.toString()).to.eq(
      (10_000n + baseToQuote.amountOut).toString(),
    );

    const quoteToBase = quoteSwapExactIn(
      poolQuoteReserve,
      poolBaseReserve,
      500n,
      30n,
    ...
    baseAccount = await getAccount(
      provider.connection,
      providerBaseAta,
      undefined,
      TOKEN_2022_PROGRAM_ID,
    );
    quoteAccount = await getAccount(
      provider.connection,
      providerQuoteAta,
      undefined,
      TOKEN_2022_PROGRAM_ID,
    );
    expect(baseAccount.amount.toString()).to.eq(
      (10_000n - 1_000n + quoteToBase.amountOut).toString(),
    );
    expect(quoteAccount.amount.toString()).to.eq(
      (10_000n + baseToQuote.amountOut - 500n).toString(),
    );
  });
  ...
  it("rejects create_liquidity_pool with an invalid fee", async () => {
    ...
    const [badLpMintPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("lp_mint"), fakeCompany.toBuffer(), Buffer.from(badPoolId)],
      program.programId,
    );
